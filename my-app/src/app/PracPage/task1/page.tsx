...
import { fetchTask1ExamList } from "@/services/examService";
...
export default function PracticePage() {
  ...
  useEffect(() => {
    const getExams = async () => {
      try {
        const data = await fetchTask1ExamList();
        setExamList(data);
      } catch (error: any) {
      ...
      <div className="container_mainpage">
        ...
        <div className="sidebar">
          <button
            ...
            onClick={() => router.push("/PracPage/task1")}
          >
            Task 1
          </button>
          <button
            className="task-button"
            onClick={() => router.push("/PracPage/task2")}
          >
            Task 2
          </button>
        </div>
            ...
            {loading ? (
              ...
              currentItems.map((exam) => ( // Chỉ hiển thị các bài trong trang hiện tại
                <div 
                  key={exam.id} 
                  className="card"
                  onClick={() => router.push(`/exam/task1/${exam.id}`)}
                >
